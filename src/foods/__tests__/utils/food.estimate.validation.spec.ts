...
const valid = { kcal: 280, protein: 8, carbs: 25, fat: 18 };
...
describe('validateFoodEstimate', () => {
  ...
  it('accepts all-zero estimate (signals "unknown food")', () => {
    expect(validateFoodEstimate({ kcal: 0, protein: 0, carbs: 0, fat: 0 })).toEqual({
      kcal: 0, protein: 0, carbs: 0, fat: 0,
    });
  });

  it('accepts fractional values (per-gram estimates)', () => {
    expect(validateFoodEstimate({ kcal: 1.5, protein: 0.07, carbs: 0.15, fat: 0.08 })).toEqual({
      kcal: 1.5, protein: 0.07, carbs: 0.15, fat: 0.08,
    });
  });
...
describe('isZeroEstimate', () => {
  it('returns true when all macros are zero', () => {
    expect(isZeroEstimate({ kcal: 0, protein: 0, carbs: 0, fat: 0 })).toBe(true);
  });

  it('returns false when any macro is non-zero', () => {
    expect(isZeroEstimate({ kcal: 1, protein: 0, carbs: 0, fat: 0 })).toBe(false);
    expect(isZeroEstimate({ kcal: 0, protein: 0, carbs: 0, fat: 0.1 })).toBe(false);
  });
});
